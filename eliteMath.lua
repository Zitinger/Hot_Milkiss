...
-- version 1.00+, home at eliteGeometry

function new()
	local CMathExt = {};
	
	function CMathExt:insidePolygon(polygon, point)
	...
	function CMathExt:insideVertices(vertices, point)
		local polygon = {};
		for i=1,#vertices,2 do
			table.insert(polygon, {x=vertices[i], y=vertices[i+1]});
		end
	...
	function CMathExt:distPointToLine(x0,y0,x1,y1,x2,y2)
		--https://en.wikipedia.org/wiki/Distance_from_a_point_to_a_line
		local dx=x2-x1;
		local dy=y2-y1;
		local d = math.sqrt(dx*dx+dy*dy);
		local t = math.abs(dy*x0 - dx*y0 + x2*y1 - y2*x1);
		return t/d
	end
